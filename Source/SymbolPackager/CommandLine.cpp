...
bool tryParseFormat(const std::string &value, Format &fmt)
{
    ...
    else if (isEqualIgnoreCase(value, "ELF"))
    {
        fmt = Format_ELF;
    }
...
#else
"  --format <format> { Symbol, ELF, MSMap, GNUMap, GNUNm }. The format will be\n"
#endif
...
#else
"                    an ELF binary, a map file generated by the linker, the nm\n"
"                    tool or a previously generated symbol file.\n"
#endif
    );
}
    ...
    if (error.empty())
    {
        if (_command == Command_None)
        {
            ...
            else
            {
                ...
                case Format_ELF:
                    _command = Command_PackageELFFile;
                    break;
