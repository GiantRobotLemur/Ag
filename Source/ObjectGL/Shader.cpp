...
ShaderType Shader::getType() const
{
    ...
    GLint value = 0;
    api.getShaderIV(_shader->getName(), ShaderParameterName::ShaderType, &value);

    return static_cast<ShaderType>(value);
}

//! @brief Gets the identifier of the underlying OpenGL shader.
ShaderName Shader::getName() const
{
    return (_shader) ? _shader->getName() : ShaderName();
}
...
bool Shader::isMarkedForDeletion() const
{
    ...
    api.getShaderIV(_shader->getName(),
                    ShaderParameterName::DeleteStatus,
                    &value);
...
Ag::String Shader::getSource() const
{
    ...
    GLint length = 0;
    api.getShaderIV(_shader->getName(),
                    ShaderParameterName::ShaderSourceLength,
                    &length);
...
Ag::String Shader::getCompilationLog() const
{
    ...
    GLint length = 0;
    api.getShaderIV(_shader->getName(),
                    ShaderParameterName::InfoLogLength,
                    &length);
...
//! valid resource.
const GLAPI &Shader::verifyAccess(const char *operation) const
{
    if (_shader)
    {
