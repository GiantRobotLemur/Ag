...
#include "Ag/Sdl3Tools/OpenGL.hpp"
...
bool tryGetOpenGLCoreProfileInfo(OpenGLInfo &info)
{
    ...
    window.create("OpenGL Core Profile Detection", 640, 480,
                  SDL_WINDOW_OPENGL | SDL_WINDOW_HIDDEN);

    if (window.isBound())
    {
        // Create an OpenGL context using that window.
        GLContextUPtr context(SDL_GL_CreateContext(window.verifyAccess("Create OpenGL context")));

        // Select the context on the current thread so we can interrogate it.
        if (context && (SDL_GL_MakeCurrent(window.verifyAccess("Set OpenGL context"), context.get()) == 0))
        {
        ...
        SDL_GL_MakeCurrent(window.verifyAccess("Clear OpenGL context"), nullptr);
    }
