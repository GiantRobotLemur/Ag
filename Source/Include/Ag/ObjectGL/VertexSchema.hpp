//! @file Ag/ObjectGL/VertexSchema.hpp
...
public:
    ...
    VertexAttrib(const std::string_view &name, AttribType dataType,
                 size_t offset, int elementCount = -1);

    // Accessors
    Ag::string_cref_t getName() const;
    AttribType getDataType() const;
    ...
    // Operations
    void define(const GLAPI &api, uint32_t attributeId, size_t vertexSize) const;
private:
    // Internal Fields
    Ag::String _name;
    uint32_t _offset;
    AttribType _dataType;
...
public:
    ...
    void addAttribute(const std::string_view &name, AttribType dataType,
                      size_t offset, int elementCount = -1);
